...
export default function FacultyLayout({ children }: { children: ReactNode }) {
  ...
  const navItems = [
    { label: 'Dashboard', href: '/faculty', icon: LayoutDashboard, active: pathname === '/faculty' },
    { label: 'My Courses', href: '/faculty/courses', icon: BookOpen, active: pathname === '/faculty/courses' },
    { label: 'Course Handout (CHO)', href: '/faculty/course-handout', icon: FileText, active: pathname === '/faculty/course-handout' },
    { label: 'Coordination', href: '/faculty/coordination', icon: Crown, active: pathname === '/faculty/coordination' },
    { label: 'Lesson Plans', href: '/faculty/lesson-plans', icon: FileText, active: pathname === '/faculty/lesson-plans' },
    { label: 'Upload Content', href: '/faculty/upload', icon: Upload, active: pathname === '/faculty/upload' },
    { label: 'My Submissions', href: '/faculty/submissions', icon: FileText, active: pathname === '/faculty/submissions' },
  ]
  ...
  return (
    <div className="flex min-h-screen bg-gray-50">
      {/* Sidebar */}
      <aside className="w-64 bg-gradient-to-b from-blue-600 to-indigo-600 text-white flex flex-col fixed h-screen">
        {/* Logo */}
          ...
          {navItems.map((item) => {
            ...
            return (
              <Link
                ...
                href={item.href}
                className={`flex items-center gap-3 px-4 py-3 rounded-lg transition-colors ${
                  item.active ? 'bg-white text-blue-600' : 'text-blue-100 hover:bg-blue-500'
                }`}
                title={item.label}
              >
                <Icon className="h-5 w-5 flex-shrink-0" />
      ...
      {/* Main Content */}
      <main className="ml-64 flex-1 flex flex-col transition-all duration-300">
        {/* Top Header */}
