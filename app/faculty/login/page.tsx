...
export default function FacultyLookupPage() {
  const [email, setEmail] = useState('')
  const [loading, setLoading] = useState(false)
  const [error, setError] = useState('')
  const router = useRouter()
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (data.success && data.faculty) {
      // Redirect to their dashboard with faculty id as param or state
      router.push(`/faculty/dashboard?id=${data.faculty.id}`)
    } else {
  ...
  return (
    <div className="max-w-md mx-auto mt-24 p-8 bg-white shadow rounded-xl">
      <h1 className="text-2xl font-bold mb-4">Faculty Quick Access</h1>
      <form onSubmit={handleSubmit}>
        ...
        <button
          ...
          disabled={loading}
        >{loading ? "Loading..." : "Open My Dashboard"}</button>
      </form>
