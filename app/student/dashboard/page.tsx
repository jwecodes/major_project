  ...
  useEffect(() => {
    const init = async () => {
      try {
        ...
        const roleRes = await fetch('/api/auth/roles', {
          method: 'GET',
          credentials: 'include',
        })

        if (!roleRes.ok) {
          // Not logged in or error → go to main login
          router.push('/auth/login')
          return
        }

        const roleData = await roleRes.json()
        const role = roleData.currentRole as 'ADMIN' | 'FACULTY' | 'STUDENT' | null

        // 2) If logged in but NOT a student → redirect to their dashboard
        if (role === 'ADMIN') {
          router.push('/admin')
          return
        }
        ...
        // 3) For students: still use your existing localStorage logic
        const studentData = localStorage.getItem('studentUser')
        if (!studentData) {
          // If you no longer set this, you can instead fetch student by email via API
          router.push('/auth/login')
          return
        }
        ...
        await loadDashboardData(parsedStudent.id)
      } catch (error) {
        console.error(error)
        toast.error('Failed to initialize student dashboard')
        router.push('/auth/login')
      } finally {
        setLoading(false)
      }
